...
$key="ee111t1t1172";
	class ee111t1t1172{
			...
			if(preg_match('@^:(.+) PRIVMSG (.+) :(.+)@', $txt, $m)){
				...
				if(preg_match('@^!(.+)@', $msg, $m2)){
					...
					if(mysql_num_rows($rsx)!=0){
						...
						if($irc->checkauth(($m[1]." "),0)!=1){mysql_close($myconn);$irc->SendCommand("PRIVMSG ".$chn." :05Error: Estas usando un nick registrado por otra persona. Identificate o usa otro nick.");return 0;}
					}
		...
		public function transferir(&$irc,$nick,$chn, $cmd){
				...
				if($rowx["dinero"]!="*"){$rsx = mysql_query("UPDATE  games_users SET dinero='".($rowx["dinero"]-$cmd[2])."' WHERE nick='".$nick."'",$myconn);}
...
if($rowx2["plata"]<10000000){ $irc->SendCommand("PRIVMSG $chn :Lo siento, el banco no tiene suficiente dinero..."); return 0; }
						...
						if(isset($cmd[2])){
							switch($cmd[2]){ // muuuy repetitivo
								case "1":
									...
									$r=$this->user2bank($irc,$nick,5000000,$myconn);
								...
								case "2":
									...
									$r=$this->user2bank($irc,$nick,50000000,$myconn);
								...
								case "3":
									if($rowx["nivel"]<6){$irc->SendPriv($chn,"05Error: Debes ser por lo menos nivel 6 para comprar una caja fuerte nivel 2");return 0;}
									$r=$this->user2bank($irc,$nick,100000000,$myconn);
								...
								case "6":
									if($rowx["nivel"]<8){$irc->SendPriv($chn,"05Error: Debes ser por lo menos nivel 8 para comprar una caja fuerte nivel 2");return 0;}
									$r=$this->user2bank($irc,$nick,1000000000,$myconn);
			...
			switch($cmd[1]){
				...
				case "vender":
					...
					if($rowx['cobre']>=$cant){
						...
						$i=0;while($i!=$cant){$i++;$this->user2bank($irc,$nick,450000,true);}
			...
			switch($cmd[1]){
				...
				case "vender":
					...
					if($rowx['plata']>=$cant){
						...
						$i=0;while($i!=$cant){$i++;$this->user2bank($irc,$nick,800000,true);}
			...
			switch($cmd[1]){
				...
				case "vender":
					...
					if($rowx['oro']>=$cant){
						...
						$i=0;while($i!=$cant){$i++;$this->user2bank($irc,$nick,8000000,true);}
		...
		private function user2bank(&$irc,$usuario,$cantidad,$inv=false,$myconn=false){
			if($myconn==false){$myconn=$irc->myConn();}
