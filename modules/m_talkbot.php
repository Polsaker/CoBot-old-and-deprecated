...
class key{
	...
	public function __construct(&$core){
        $core->registerCommand("activbt", "talkbot", "Activa el bottalk en un canal. Sintaxis: activbt <canal> <cleverbot|jaberwacky|pandorabot>");
        $core->registerCommand("deactivbt", "talkbot", "Desctiva el bottalk en un canal. Sintaxis: deactivbt <canal>");
        $core->registerMessageHandler('PRIVMSG', "talkbot", "recvmsg");
	...
	public function recvmsg($irc, $data, $core){
		if(isset($this->talkchans[$data->channel])){
			$t = html_entity_decode($this->bots[$this->talkchans[$data->channel]]->think($data->message));
			$irc->message(SMARTIRC_TYPE_CHANNEL, $data->channel, $t);
		}
	}
	
	public function activbt($irc, $data, $core){
		if(!isset($data->messageex[2])){return 0;}
		switch($data->messageex[2]){
			case "cleverbot":
				$this->talkchans[$data->messageex[1]] = 1;
				break;
			case "jabberwacky":
				$this->talkchans[$data->messageex[1]] = 2;
				break;
			case "pandorabot": 
				$this->talkchans[$data->messageex[1]] = 3;
				break;
		}
	}
	
	public function deactivbt($irc, $data, $core){
		if(isset($data->messageex[2])){unset($this->talkchans[$data->messageex[1]]);}
	}
}
